...
library(MASS); library(Matrix); library(sparseMVN); library(matrixcalc); library(snow); library(Rcpp); library(foreach); library(doParallel); library(igraph)
...
cv.vote = function(pos, cv, result.cv, d.l, lambda.c, d.pos, Omega.t, edge.t, edge, overlap.edge){
  
  d.l = d.l[d.pos]; edge = edge[, d.pos, ]; overlap.edge = overlap.edge[, d.pos, ]
  
  p = dim(Omega.t)[1]; N = dim(Omega.t)[3]; fold = length(result.cv); K = length(pos)
  ...
  if(length(d.pos)!=1){
    ...
    cv = apply(cv[, d.pos, , ], c(1,2,3), sum)
    
    L = dim(cv)[1]; D = dim(cv)[2]; K = dim(cv)[3]
    
    lambda_d.index = matrix(0, 2, K)
    for(k in 1:K){
      index = which(cv[, , k] == min(cv[, , k]), arr.ind = T)
    ...
    FDR.list = (1-overlap.edge.cv/edge.cv)[-c(1, K)]
    FDR.d.list = (1-overlap.edge.d.cv/edge.d.cv)[-c(1, K)]
    FDR.avg.list = (1-overlap.edge.avg.cv/edge.avg.cv)[-c(1, K)]
    power.list = (overlap.edge.cv/edge.t[pos])[-c(1, K)]
    power.d.list = (overlap.edge.d.cv/edge.t[pos])[-c(1, K)]
    power.avg.list = (overlap.edge.avg.cv/edge.t[pos])[-c(1, K)]
    ...
    FDR.cv[, 2] = c(sd(FDR.list), sd(FDR.d.list), sd(FDR.avg.list))/sqrt(K-2)
    power.cv[, 2] = c(sd(power.list), sd(power.d.list), sd(power.avg.list))/sqrt(K-2)
    ...
    cv.score = c(mean(s.cv[-c(1, K)]), mean(s.d.cv[-c(1, K)]), mean(s.avg.cv[-c(1, K)]))
    
    for(i in 1:fold){
      
      Omega.rf.list = result.cv[[i]][[3]][, d.pos, ]
    ...
    Omega.cv = (apply(Omega.cv!=0, c(1,2,3), sum)==fold) 
    Omega.d.cv = (apply(Omega.d.cv!=0, c(1,2,3), sum)==fold)
    Omega.avg.cv = (apply(Omega.avg.cv!=0, c(1,2,3), sum)==fold)
    ...
    FDR.list = (1-overlap.edge.cv/edge.cv)[-c(1, K)]; FDR.list[is.nan(FDR.list)] = 0
    FDR.d.list = (1-overlap.edge.d.cv/edge.d.cv)[-c(1, K)]; FDR.d.list[is.nan(FDR.d.list)] = 0
    FDR.avg.list = (1-overlap.edge.avg.cv/edge.avg.cv)[-c(1, K)]; FDR.avg.list[is.nan(FDR.avg.list)] = 0
    power.list = (overlap.edge.cv/edge.t[pos])[-c(1, K)]
    power.d.list = (overlap.edge.d.cv/edge.t[pos])[-c(1, K)]
    power.avg.list = (overlap.edge.avg.cv/edge.t[pos])[-c(1, K)]
    ...
    FDR.cv.vote[, 2] = c(sd(FDR.list), sd(FDR.d.list), sd(FDR.avg.list))/sqrt(K-2)
    power.cv.vote[, 2] = c(sd(power.list), sd(power.d.list), sd(power.avg.list))/sqrt(K-2)
    ...
    cv = apply(cv[, d.pos, , ], c(1,2), sum)
    
    L = dim(cv)[1]; K = dim(cv)[2]
    
    d.lambda.index = sapply(1:K, function(k) which.min(cv[, k]))
    d.cv[1] = d.l; d.cv[-1] = lambda.c[d.lambda.index]
    
    cv.avg = apply(cv, 1, mean)
    lambda_d.avg.index = which(cv.avg == min(cv.avg))
    lambda_d.avg.cv = c(d.l, lambda.c[lambda_d.avg.index])
    
    for(k in 1:K){
      
      edge.d.cv[k] = edge[d.lambda.index[k], k]
      overlap.edge.d.cv[k] = overlap.edge[d.lambda.index[k], k]
      s.d.cv[k] = cv[d.lambda.index[k], k]
    }
    edge.avg.cv = edge[lambda_d.avg.index, ]
    overlap.edge.avg.cv = overlap.edge[lambda_d.avg.index, ]
    s.avg.cv = cv[lambda_d.avg.index, ]
    ...
    FDR.cv[-1, 1] = c(mean(FDR.d.list), mean(FDR.avg.list))
    power.cv[-1, 1] = c(mean(power.d.list), mean(power.avg.list))
    FDR.cv[-1, 2] = c(sd(FDR.d.list), sd(FDR.avg.list))/sqrt(K-2)
    power.cv[-1, 2] = c(sd(power.d.list), sd(power.avg.list))/sqrt(K-2)
    F1.cv[-1, 1] = c(mean(2*(1-FDR.d.list)*power.d.list/(1-FDR.d.list+power.d.list)), mean(2*(1-FDR.avg.list)*power.avg.list/(1-FDR.avg.list+power.avg.list)))
    cv.score = c(mean(s.d.cv[-c(1, K)]), mean(s.avg.cv[-c(1, K)]))
    ...
    for(i in 1:fold){
      ...
      for(k in 1:K){
        ...
        Omega.d.cv[, , k, i] = as.matrix(Omega.rf.list[[d.lambda.index[k], k]])
        Omega.avg.cv[, , k, i] = as.matrix(Omega.rf.list[[lambda_d.avg.index, k]])  
      }
    }
    ...
    FDR.cv.vote[-1, 1] = c(mean(FDR.d.list), mean(FDR.avg.list))
    power.cv.vote[-1, 1] = c(mean(power.d.list), mean(power.avg.list))
    FDR.cv.vote[-1, 2] = c(sd(FDR.d.list), sd(FDR.avg.list))/sqrt(K-2)
    power.cv.vote[-1, 2] = c(sd(power.d.list), sd(power.avg.list))/sqrt(K-2)
    F1.d = 2*(1-FDR.d.list)*power.d.list/(1-FDR.d.list+power.d.list); F1.d[is.nan(F1.d)] = 0
    F1.avg = 2*(1-FDR.avg.list)*power.avg.list/(1-FDR.avg.list+power.avg.list); F1.avg[is.nan(F1.avg)] = 0
    F1.cv[-1, 2] = c(mean(F1.d), mean(F1.avg))
  }
...
optimal.select = function(d.l, lambda.c, d.pos, edge, overlap.edge, edge.t){
  
  if(length(d.pos)!=1){
    
    d.l = d.l[d.pos]; edge = edge[, d.pos, ]; overlap.edge = overlap.edge[, d.pos, ]
    
    D = length(d.l); L = length(lambda.c); K = dim(edge)[3]; N = length(edge.t); pos = (1:K-1)/(K-1)*(N-1)+1
    ...
    lambda_d.index = matrix(0, 2, K)
    for(k in 1:K){
      index = which(F1[, , k] == max(F1[, , k]), arr.ind = T)
      if(nrow(index)>1){index = matrix(index[nrow(index), ], c(1, 2))}
      lambda_d.index[, k] = index
    ...
    FDR.opt[, 1] = c(mean(sapply(2:(K-1), function(k) FDR[lambda_d.index[1, k], lambda_d.index[2, k], k])), mean(sapply(2:(K-1), function(k) FDR[d.lambda.index[k], d.index, k])), mean(FDR[lambda_d.avg.index[1], lambda_d.avg.index[2], -c(1, K)]))
    FDR.opt[, 2] = c(sd(sapply(2:(K-1), function(k) FDR[lambda_d.index[1, k], lambda_d.index[2, k], k])),sd(sapply(2:(K-1), function(k) FDR[d.lambda.index[k], d.index, k])), sd(FDR[lambda_d.avg.index[1], lambda_d.avg.index[2], -c(1, K)]))/sqrt(K-2)
    power.opt[, 1] = c(mean(sapply(2:(K-1), function(k) power[lambda_d.index[1, k], lambda_d.index[2, k], k])), mean(sapply(2:(K-1), function(k) power[d.lambda.index[k], d.index, k])), mean(power[lambda_d.avg.index[1], lambda_d.avg.index[2], -c(1, K)]))
    power.opt[, 2] = c(sd(sapply(2:(K-1), function(k) power[lambda_d.index[1, k], lambda_d.index[2, k], k])),sd(sapply(2:(K-1), function(k) power[d.lambda.index[k], d.index, k])), sd(power[lambda_d.avg.index[1], lambda_d.avg.index[2], -c(1, K)]))/sqrt(K-2)
    F1.opt = c(mean(sapply(2:(K-1), function(k) F1[lambda_d.index[1, k], lambda_d.index[2, k], k])), mean(sapply(2:(K-1), function(k) F1[d.lambda.index[k], d.index, k])), mean(F1[lambda_d.avg.index[1], lambda_d.avg.index[2], -c(1, K)]))
    
  }else{
    
    d.l = d.l[d.pos]; edge = edge[, d.pos, ]; overlap.edge = overlap.edge[, d.pos, ]
    
    L = length(lambda.c); K = dim(edge)[2]; N = length(edge.t); pos = (1:K-1)/(K-1)*(N-1)+1
    
    FDR = matrix(0, L, K); power = matrix(0, L, K)
    for(l in 1:L){
      FDR[l, ] = 1-overlap.edge[l, ]/edge[l, ]; power[l, ] = overlap.edge[l, ]/edge.t[pos]
    }
    FDR[is.nan(FDR)] = 1
    ...
    d.lambda.index = sapply(1:K, function(k) which.max(F1[, k]))
    d.opt[1] = d.l; d.opt[-1] = lambda.c[d.lambda.index]
    
    F1.avg = apply(F1, 1, mean)
    lambda_d.avg.index = which(F1.avg == max(F1.avg))
    lambda_d.avg.opt = c(d.l, lambda.c[lambda_d.avg.index])
    
    FDR.opt[-1, 1] = c(mean(sapply(2:(K-1), function(k) FDR[d.lambda.index[k], k])), mean(FDR[lambda_d.avg.index, -c(1, K)]))
    FDR.opt[-1, 2] = c(sd(sapply(2:(K-1), function(k) FDR[d.lambda.index[k], k])), sd(FDR[lambda_d.avg.index, -c(1, K)]))/sqrt(K-2)
    power.opt[-1, 1] = c(mean(sapply(2:(K-1), function(k) power[d.lambda.index[k], k])), mean(power[lambda_d.avg.index, -c(1, K)]))
    power.opt[-1, 2] = c(sd(sapply(2:(K-1), function(k) power[d.lambda.index[k], k])), sd(power[lambda_d.avg.index, -c(1, K)]))/sqrt(K-2)
    F1.opt = c(mean(sapply(2:(K-1), function(k) F1[d.lambda.index[k], k])), mean(F1[lambda_d.avg.index, -c(1, K)]))
